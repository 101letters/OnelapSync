...
LOCK_FILE = BASE_DIR / "magene_sync_v3.lock"
...
def setup_logger(verbose: bool = False):
    ensure_dirs()
    logger = logging.getLogger("magene_sync_v3")
...
class MageneStravaSyncV3:
    def __init__(self, config_path: Path, state_path: Path, logger: logging.Logger):
        ...
        self.analysis_max_hr = int(os.environ.get("ANALYSIS_MAX_HR", analysis_cfg.get("max_hr", 190)))
    ...
    def download_fit(self, record_id: str) -> bytes:
        ...
        if not file_key:
            raise RuntimeError(f"活动 {record_id} 无 fileKey")
    ...
    def refresh_strava_token(self) -> str:
        ...
        old_token = os.environ.get("STRAVA_ACCESS_TOKEN", "")
        if old_token:
    ...
    def _update_strava_tokens(self, access_token: str, new_refresh: str, old_refresh: str):
        """更新 access token，写回 .env。refresh token 变化时同步更新。"""
        self.strava_access_token = access_token

        env_content = ENV_FILE.read_text(encoding="utf-8")
        old_access = os.environ.get("STRAVA_ACCESS_TOKEN", "")
    ...
    def upload_to_strava(self, fit_data: bytes, activity_name: str) -> int | None:
        """
        上传 FIT 到 Strava，轮询等待完成，返回 activity_id。
        使用 Strava uploads API 原生上传。
        """
        # 1. 写入临时文件
        ...
        try:
            # 2. 上传（FIT 文件名用纯 ASCII，中文名走 name 字段）
            ...
            if resp.status_code != 201:
                ...
                if "duplicate" in body.lower():
                    import re as _re
                    dup_id = None
                    m = _re.search(r"/activities/(\d+)", body)
                    if m:
            ...
            for poll in range(30):
                ...
                if pr.status_code != 200:
                    self.logger.warning(f"轮询出错: {pr.status_code}")
                    continue
                ...
                if status == "Your activity is ready.":
                    ...
                    return activity_id
                elif error and ("duplicate of" in (error or "").lower() or "duplicate" in (error or "").lower()):
                    # 重复活动（不管 status 是什么，只要 error 包含 duplicate 就算）
                    import re as _re
                    dup_id = None
                    m = _re.search(r"/activities/(\d+)", (error or ""))
                    if m:
                        dup_id = int(m.group(1))
                    ...
                    self.logger.warning(f"上传状态: {status}, 错误: {error}")
                else:
                    self.logger.debug(f"上传状态: {status}")
    ...
    def _is_on_strava(self, onelap_act: dict, strava_acts: list[dict]) -> bool:
        ...
        try:
            ...
            onelap_ts = onelap_ts.replace(tzinfo=timezone(timedelta(hours=8)))
    ...
    def run(self, days: int = 1, dry_run: bool = False, force: bool = False, compensate: bool = False):
        ...
        self.logger.info(
            f"开始同步 V3 (days={effective_days}, dry_run={dry_run}, force={force}, compensate={compensate})"
        )
        ...
        # 5. 过滤出需要同步的活动
        synced_set = set(self.state.get("activities", {}).keys())
        to_sync = []
        skipped_too_short = 0
        ...
        for i, activity in enumerate(to_sync[:self.max_uploads_per_run]):
            ...
            try:
                ...
                strava_aid = self.upload_to_strava(fit_data, display_name)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Magene -> Strava 同步 (V3 - 顽鹿OTM + Strava直连)")
